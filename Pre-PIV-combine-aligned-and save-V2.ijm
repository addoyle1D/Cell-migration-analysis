...
run("Make Composite");saveName2=getTitle();rename("AO");

//Concatenate and align the combined file
run("Concatenate...", "open image1=AO image2=Main");
rename("Combined");
run("Correct 3D drift", "channel=2 only=7500 lowest=1 highest=200");
...
run("Crop");
close("MIN_C2-DUP");
close("MIN_MIN_C2-DUP");close("registered time points");

//Create a MIP image of the  3-color stack and save both versions
selectWindow("registered time points-1");
run("Z Project...", "projection=[Max Intensity] all");
selectWindow("MAX_registered time points-1");
run("Correct 3D drift", "channel=2 multi_time_scale sub_pixel edge_enhance only=10000 lowest=1 highest=1");

//Create a mask and ROi to crop the images
selectWindow("registered time points");run("Duplicate...", "title=DUP2 duplicate");
selectWindow("DUP2");
run("Split Channels");
selectWindow ("C1-DUP2");close();selectWindow ("C3-DUP2");close();
selectWindow ("C2-DUP2");
run("Z Project...", "projection=[Min Intensity] all");selectWindow("MIN_C2-DUP2");
setThreshold(1, 65535);
setTool("wand");
doWand(250,250);
run("ROI Manager...");
...
run("Crop");
close("MIN_C2-DUP2");
saveAs("Tiff", output+"AL_"+saveName3);
selectWindow("registered time points");saveAs("Tiff", output+ "AL_MAX_"+saveName3);

//Split the cropped image and save the Max version of each 
rename("Stack");
run("Split Channels");
selectWindow ("C2-Stack");
roiManager("Select", 1);run("Crop");
//run("Z Project...", "projection=[Max Intensity] all");
...
roiManager("Select", 1);run("Crop");
//run("Z Project...", "projection=[Max Intensity] all");
setSlice(1);run("Delete Slice", "delete=frame");run("Enhance Contrast", "saturated=0.30");
