...
//rename("Combined");
run("Correct 3D drift", "channel=2 only=7500 lowest=1 highest=26");

//Create a substack
selectWindow("registered time points");
waitForUser("Select top and bottom for substack");
run("Make Substack...");close("registered time points"); close("Main");
...
//roiManager("Select", 0);run("Crop");
run("Z Project...", "projection=[Max Intensity] all"); run("Enhance Contrast", "saturated=0.30");
saveAs("Tiff", targetD3+"AL_MAX_"+saveName3+"_ECM");close("AL_MAX_"+saveName3+"_ECM");

selectWindow ("C1-Stack");
//roiManager("Select", 0);run("Crop");
run("Z Project...", "projection=[Max Intensity] all");setSlice(1);run("Delete Slice", "delete=frame");run("Enhance Contrast", "saturated=0.30");
...
run("Close All");roiManager("Deselect");roiManager("Delete");setTool("rectangle");
